...
if __name__ == '__main__':
    ...
    # initialize opt part
    OptSupportConf.mask = common_base_funs.get_file_lines(OptSupportConf.mask_file)
    OptSupportConf.mask = [_.split(',') for _ in OptSupportConf.mask]
    OptSupportConf.mask = np.array([[int(__) for __ in _] for _ in OptSupportConf.mask])
    OptSupportConf.normalizer = MinMaxScaler()
    OptSupportConf.normalizer.fit(np.loadtxt(OptSupportConf.normalize_file, delimiter=','))
    OptSupportConf.test_opt_list = common_base_funs.get_file_lines(OptSupportConf.test_opt_file)
    OptSupportConf.train_opt_list = common_base_funs.get_file_lines(OptSupportConf.train_opt_file)
    # Here, we do not use a static class field to predict. For each process, we load the model once.
    # OptSupportConf.model = xgb.XGBClassifier()
    # OptSupportConf.model.load_model(OptSupportConf.model_file)
    if MainProcessConf.test_compiler_type == MainProcessConf.compiler_types[0]:  # GCC
        OptSupportConf.mapper = OptSupportConf.get_gcc_mapping(OptSupportConf.test_opt_list, OptSupportConf.train_opt_list)
    if MainProcessConf.test_compiler_type == MainProcessConf.compiler_types[1]:  # LLVM
        OptSupportConf.mapper = OptSupportConf.get_llvm_mapping(OptSupportConf.test_opt_list, OptSupportConf.train_opt_list)
    assert OptSupportConf.mapper is not None
