...
from main_configure_approach import GeneralOperationConf, EnvironmentConf, CsmithConfigurationConf, OptSupportConf
...
class LLVMTestComProcess(TestProcess):
    def do_test(self):
        return self.do_dol()
    ...
    def test_com_compile(self, opt):
        bc_res = self.work_dir + LLVMTestConf.bc_res
        bc_err = self.work_dir + LLVMTestConf.bc_err
        bc_timeout_recorder = self.work_dir + LLVMTestConf.bc_timeout_recorder
        bc_crash_recorder = self.work_dir + LLVMTestConf.bc_crash_recorder
        common_base_funs.rm_file(bc_res)
        common_base_funs.rm_file(bc_err)
        cmd = ' '.join([self.llvm_clang, self.libs, '-O3 -mllvm -disable-llvm-optzns -c -emit-llvm', self.src, '-o',
                        bc_res, '2>', bc_err])
        t = time.time()
        time_in = common_base_funs.execmd_limit_time(cmd, self.time_limit)
        t = time.time() - t
        if not time_in:
            err = common_base_funs.get_file_content(bc_err)
            common_base_funs.put_file_content(bc_timeout_recorder, opt + '\n' + err)
            return LLVMTestState.compile_timeout, t  # here we use common status as clang compile
        if not os.path.exists(bc_res) or os.path.getsize(bc_res) == 0:
            err = common_base_funs.get_file_content(bc_err)
            common_base_funs.put_file_content(bc_crash_recorder, opt + '\n' + err)
            return LLVMTestState.compile_crash, t
        return LLVMTestState.success, t

    def test_com_opt(self, opt):
        bc_res = self.work_dir + LLVMTestConf.bc_res
        bc_opt_res = self.work_dir + LLVMTestConf.bc_opt_res
        bc_opt_err = self.work_dir + LLVMTestConf.bc_opt_err
        bc_opt_timeout_recorder = self.work_dir + LLVMTestConf.bc_opt_timeout_recorder
        bc_opt_crash_recorder = self.work_dir + LLVMTestConf.bc_opt_crash_recorder
        common_base_funs.rm_file(bc_opt_res)
        common_base_funs.rm_file(bc_opt_err)
        cmd = ' '.join([self.llvm_opt, opt, bc_res, '-o', bc_opt_res, '2>', bc_opt_err])
        t = time.time()
        time_in = common_base_funs.execmd_limit_time(cmd, self.time_limit)
        t = time.time() - t
        if not time_in:
            err = common_base_funs.get_file_content(bc_opt_err)
            common_base_funs.put_file_content(bc_opt_timeout_recorder, opt + '\n' + err)
            return LLVMTestState.compile_timeout, t
        if not os.path.exists(bc_opt_res) or os.path.getsize(bc_opt_res) == 0:
            err = common_base_funs.get_file_content(bc_opt_err)
            common_base_funs.put_file_content(bc_opt_crash_recorder, opt + '\n' + err)
            return LLVMTestState.compile_crash, t
        return LLVMTestState.success, t

    def test_com_ass(self, opt):
        bc_opt_res = self.work_dir + LLVMTestConf.bc_opt_res
        com_res = self.work_dir + LLVMTestConf.compile_res
        com_err = self.work_dir + LLVMTestConf.compile_err
        com_timeout_recorder = self.work_dir + LLVMTestConf.compile_timeout_recorder
        com_crash_recorder = self.work_dir + LLVMTestConf.compile_crash_recorder
        common_base_funs.rm_file(com_res)
        common_base_funs.rm_file(com_err)
        cmd = ' '.join([self.llvm_clang, bc_opt_res, '-o', com_res, '2>', com_err])
        t = time.time()
        time_in = common_base_funs.execmd_limit_time(cmd, self.time_limit)
        t = time.time() - t
        if not time_in:
            err = common_base_funs.get_file_content(com_err)
            common_base_funs.put_file_content(com_timeout_recorder, opt + '\n' + err)
            return LLVMTestState.compile_timeout, t
        if not os.path.exists(bc_opt_res) or os.path.getsize(bc_opt_res) == 0:
            err = common_base_funs.get_file_content(com_err)
            common_base_funs.put_file_content(com_crash_recorder, opt + '\n' + err)
            return LLVMTestState.compile_crash, t
        return LLVMTestState.success, t

    def test_com_execute(self, opt, execute_res):
    ...
    def do_dol(self):
        ...
        res, t = self.test_com_compile('')
        spts.append(t)
        if res != LLVMTestState.success:
            return res, sum(spts)

        res, t = self.test_com_opt('')
        spts.append(t)
        if res != LLVMTestState.success:
            return res, sum(spts)

        res, t = self.test_com_ass('')
        spts.append(t)
        if res != LLVMTestState.success:
            return res, sum(spts)

        res, t = self.test_com_execute('', O0_execute_res)
        spts.append(t)
        if res != LLVMTestState.success:
            return res, sum(spts)

        for o in self.opts:
            res, t = self.test_com_compile(o)
            spts.append(t)
            if res != LLVMTestState.success:
                return res, sum(spts)

            res, t = self.test_com_opt(o)
            spts.append(t)
            if res != LLVMTestState.success:
                return res, sum(spts)

            res, t = self.test_com_ass(o)
            spts.append(t)
            if res != LLVMTestState.success:
                return res, sum(spts)

            res, t = self.test_com_execute(o, o_execute_res)
            spts.append(t)
            if res != LLVMTestState.success:
                return res, sum(spts)
...
def test_process(conf_id, compiler_type, todo_queue, program_info_queue, timeout_queue):
    ...
    xgb_model.load_model(OptSupportConf.model_file)
    while not todo_queue.empty():
        ...
        opt_prob_file = common_base_funs.add_sep(test_dir) + 'rec.opt.prob.txt'
        opt_prob = opt_support.recommend_opt(src, src_feature=feature_file, xgb_model=xgb_model)
        rec_opt = []
        for op in opt_prob:
            o = op[0]
            prob = op[1]
            common_base_funs.put_file_content(opt_prob_file, str(prob) + ':' + str(o) + '\n')
            if prob >= OptSupportConf.discard_prob:
                rec_opt.append(o)
        """
        sometimes, ${rec_opt} will be an empty list, 
        that means all of the random generated optimization sequences have very low probability to trigger a compiler bug.
        in this case, even ${rec_opt} is an empty list, 
        this program will still be compiled and executed under O0 to determine whether it is a timeout program(harmful program).
        """
        if compiler_type == MainProcessConf.compiler_types[0]:
            tp = GCCTestProcess(libs=[GCCTestConf.csmith_lib], opts=rec_opt, src=src, work_dir=test_dir)
        if compiler_type == MainProcessConf.compiler_types[1]:
            tp = LLVMTestComProcess(libs=[LLVMTestConf.csmith_lib], opts=rec_opt, src=src, work_dir=test_dir)
